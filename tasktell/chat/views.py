...
class ChatDetailsView(LoginRequiredMixin, FormMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        my_chat = Chat.objects.get(members_id=user_member.pk)
        context['chat_list'] = Chat.objects.exclude(members_id=user_member.pk)
        context['current_chat'] = current_chat
        received_messages = Messages.objects.filter(chat_id=my_chat.pk, created_by=current_chat.members.user.pk)
        sent_messages = Messages.objects.filter(chat_id=current_chat.pk, created_by=self.request.user.pk)
